...
export function LeftSidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col gap-4 sticky top-4 h-fit">
      {/* Profile Card */}
      <div className="feed-card flex flex-col items-center text-center cursor-pointer" onClick={() => navigate("/profile")}>
        <div className="relative mb-3">
          <img src={currentUser.avatar} alt={currentUser.name} className="h-16 w-16 rounded-full bg-muted" />
          {currentUser.isPro && (
        ...
        <div className="flex items-center gap-4 w-full">
          <div className="flex-1 text-center">
            <div className="karma-badge justify-center"><Star className="h-3.5 w-3.5 fill-karma" />{currentUser.karma}</div>
            <span className="text-[10px] text-muted-foreground mt-1 block">Karma</span>
          </div>
          <div className="w-px h-8 bg-border" />
          <div className="flex-1 text-center">
            <div className="flex items-center justify-center gap-1 text-sm font-semibold text-success"><Shield className="h-3.5 w-3.5" />Verified</div>
      ...
      <div className="feed-card bg-gradient-to-br from-accent/10 to-accent/5 border-accent/20">
        ...
        <p className="text-xs text-muted-foreground mb-3">Unlock live locations, priority posts, and unlimited requests.</p>
